...
### 추가 발견 — 삭제가 유지 안 됨(Google Drive 복원) + `prune-orphans` (2026-06-11)
- **증상**: 위에서 지운 orphan 엔티티가 다시 나타남(옵시디언). 분류 자체는 정상(마커·정식 노트는 task)인데 **삭제한 파일이 되살아남**.
- **원인**: 볼트가 Google Drive(CloudStorage)라 삭제가 ~30분 뒤 동기화로 **복원**됨(파일 birth time만 새로, modify time은 옛 값 보존; launchd 무활동). 1회성 수동 삭제는 Drive 복원을 못 버팀. 총 **유령 5개**(song/artist/playlist 엔티티 + journal→product/task 재분류 후 남은 옛 Daily 사본).
- **조치**: 반복 실행 가능한 `mem.py prune-orphans` 신설(dry-run 기본, `--apply` 시 백업 후 삭제). 탐지: (a) 음악 폴더인데 source_raw 원본이 비음악으로 분류 → music orphan, (b) 그 외 미참조인데 source_raw의 정식본이 다른 곳 → ghost duplicate, (c) 나머지 미참조 → 보고만(수동 노트 보호). **멱등**이라 Drive가 또 복원해도 재실행으로 자가치유.
- **중요 버그 수정(NFC/NFD)**: macOS는 파일명 NFD, 마커 JSON은 NFC라 경로 비교가 어긋나 **참조된 정식 노트(stock)를 유령으로 오판**할 뻔함 → `unicodedata.normalize("NFC", …)`로 정규화. (dry-run 안전 점검이 오삭제를 사전 차단)
- 실볼트 유령 5개 정리 완료(백업). `tests/test_prune_orphans.py` 7건. Drive 동기화 설정은 미변경(사용자 결정) — 재발 시 `prune-orphans --apply` 재실행으로 흡수.

...
이 문서는 `my-life-memory` 프로젝트를 이어받는 사용자, Codex, Claude Code, Antigravity, MCP worker, 또는 미래의 AI agent가 프로젝트의 의도와 현재 상태를 빠르게 이해하고 작업을 이어갈 수 있도록 만든 핸드오프 문서다.
...
```
/Users/sangmin/Library/CloudStorage/GoogleDrive-iamsangmin@gmail.com/내 드라이브/[ Cloud Notes ]/Obsidian Vault/my-memory-vault
```
...
### 7.5 `scripts/scheduled_lint.sh` ★신규

미처리 raw note 감지 → AI lint job 등록 + rule-based lint 즉시 실행.

```bash
# MacBook launchd 설치 (매일 22:00)
bash scripts/install-launchd.sh

# 수동 실행
bash scripts/scheduled_lint.sh

...
>
> 📱 **"이 맥에서 클로드코드를 써도 되나"의 답은 "조치 8"에 있다.**
> 실행 방식별 실측 결과: **데스크탑 앱의 코드 세션은 안전**, 그냥 `claude`는 충돌,
> `--bare`는 로그인 불가. 빠른 점검은 `pgrep -f "bun server.ts"` (1개면 정상).
...
교체 도구를 새로 만들었다: **`scripts/ccc-token-swap.sh`** (정본)
실행은 **`bash ~/ccc-token-swap.sh`** — 홈의 그 경로는 위 정본을 가리키는 심볼릭 링크다.
...
## 조치 8 (2026-08-03) — 클로드코드 공존 규칙: 실행 방식별 실측

**배경:** 텔레그램은 봇 하나당 수신기(getUpdates 폴링)를 **하나만** 허용한다.
클로드코드는 텔레그램 플러그인이 로드되면 자동으로 수신기(bun)를 띄우므로,
"이 맥에서 클로드코드를 쓰면 CCC봇이 죽는가"가 실사용상 중요한 질문이 된다.

기존 문서에는 "claude를 따로 실행하는 자동화 + 플러그인 동시 로드 금지"라는 **원칙**만 있었고,
실행 방식별로 실제 어떤지는 측정된 바가 없었다. 아래는 **직접 실행해 측정한 결과**다.

### 안전한 측정 방법 (CCC봇에 영향 없이 검증하는 법)

`TELEGRAM_STATE_DIR`을 **빈 임시 폴더**로 지정하면 `server.ts`가 `.env`를 찾지 못해
토큰 없이 `process.exit(1)` 한다. **폴링 시작 전에 죽으므로 CCC봇과 충돌이 원천적으로 불가능**하다.

수신기가 떴는지는 `ccc-telegram-mcp.log`의 **`"MCP 서버 시작"` 줄 증가분**으로 판정한다
(플러그인 MCP는 `ccc-mcp-wrapper.sh`를 거치고, 래퍼가 기동 때마다 이 줄을 남긴다).

```bash
TELEGRAM_STATE_DIR=$(mktemp -d) <검증할 claude 명령> -p '...'
```

### 측정 결과

| 실행 방식 | 수신기 기동 | 로그인 | 스킬 | 판정 |
|---|---|---|---|---|
| **클로드 데스크탑 앱의 코드 세션** | **안 뜸** | ✅ | ✅ | ✅ **안전 — 권장** |
| `claude --strict-mcp-config --mcp-config '{"mcpServers":{}}'` | 안 뜸 | ✅ | ✅ | ✅ 안전(터미널용) |
| 그냥 `claude` | **뜸** | ✅ | ✅ | ❌ 충돌 |
| `claude --bare` | 안 뜸 | **❌ Not logged in** | — | ❌ 사용 불가 |
| `ccc` 중복 실행 | 뜸 | ✅ | ✅ | ❌ 절대 금지 |

**`--bare`는 쓸 수 없다.** `--help`에 명시된 대로 `--bare`는 **keychain read를 건너뛰고**
인증을 `ANTHROPIC_API_KEY`로만 한다. 이 맥은 키체인 네이티브 인증(조치 4)이므로
`Not logged in · Please run /login`으로 실패한다. 과거 keepalive가 `--bare`를 썼다는 사실은
근거가 되지 않는다 — 그 keepalive는 2026-07-23에 폐지됐다.

### 데스크탑 앱이 안전한 이유

앱은 `~/.local/bin/claude`가 아니라 **자체 내장본**을 실행한다:

```
터미널:      ~/.local/bin/claude
데스크탑 앱:  ~/Library/Application Support/Claude/claude-code/<버전>/claude.app/Contents/MacOS/claude
```

따라서 `~/.claude/settings.json`의 `enabledPlugins`(텔레그램 ON)를 따라가지 않는다.

**실측(2026-08-03 20:52, 앱 내장본 2.1.219):** 코드 세션을 연 상태에서
bun은 1개(CCC봇 소속) 유지, 409는 0건, 세션 도구 목록에 `mcp__plugin_telegram*` **0건**.

> ⚠️ **버전 의존적일 수 있다.** 2026-06-03·06-10에 열린 데스크탑 세션(앱 내장본 2.1.160 무렵)의
> 기록에는 `mcp__plugin_telegram_telegram__*` 도구가 **도구 목록에 등재되어 있었다.**
> 즉 예전 버전은 플러그인을 로드했다. **앱이 크게 업데이트된 뒤 CCC봇이 이상하면 이것을 먼저 의심할 것.**

### 빠른 자가 점검

```bash
pgrep -f "bun server.ts"    # 1개면 정상, 2개 이상이면 충돌 중
```

### 기각된 안 — 다시 시도하지 말 것

**"전역에서 텔레그램 플러그인을 끄고 CCC봇만 예외로 켠다"** 는 현재 구조에서 불가능하다.

| 시도 | 결과 |
|---|---|
| 플러그인 OFF + 그냥 `claude` | 수신기 안 뜸 (원하는 효과는 나옴) |
| 플러그인 OFF + `--channels plugin:telegram@...` | **수신기 안 뜸 → CCC봇도 같이 죽는다** |
| 플러그인 OFF + `--plugin-dir <플러그인경로>` 단독 | 플러그인은 로드되나 채널 모드가 아님 |
| 플러그인 OFF + `--plugin-dir` + `--channels` | 로드 실패 |

`--channels`는 **꺼진 플러그인을 되살리지 못한다.** 전역 차단은 CCC봇 사망과 직결되므로 채택 불가.

### 실사용 지침

| 하고 싶은 일 | 방법 |
|---|---|
| 코드 작업 / 클로드코드 | **클로드 데스크탑 앱** |
| 굳이 터미널에서 | `claude --strict-mcp-config --mcp-config '{"mcpServers":{}}'` |
| 볼트 작업(`/mem-lint` 등) | 데스크탑 앱, 또는 텔레그램으로 CCC봇에게 |
| CCC봇 | tmux에서 자동 운영. 손대지 않는다 |
